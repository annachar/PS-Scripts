param (
    ...
    $localPath = "\\192.168.131.10\b2kcy\Channel Management\RESOLUTION GR\1. Approved\",
    $localPathArchive = "\\192.168.131.10\b2kcy\Channel Management\RESOLUTION GR\2. Pending Approval\",
    $remotePath = "/In/",
    [Switch]
    $delete,
    [Switch]
...
try
{
 ...
 foreach($localFile in Get-ChildItem $localPath){
       ...
       Move-Item -Path $($localPath+"*") -Destination $localPathArchive 
     ...
     # Remove-Item -Path $($localPath+"*") -Force -Recurse
       ...
       # Remove-Item -Path $($localPath+"*") -Force
        ...
        if($bodyMessage -ne "")
        {
            ...
            $Body = $style + "<body>Dear RU<br>I have sent you a proposal for your review with my comments.<br><br>$bodyMessage<br><br>Thank you, <br>Elena Constantinou</body>"
            $Body = $Body + $html
            ...
            $mailParams = @{
                ...
                To                         = @('resolutionsunit@b2kapital.gr', '<ech@b2kapital.com.cy>', '<gma@b2kapital.com.cy>','eco@b2kapital.com.cy' ) #
