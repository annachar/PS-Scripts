...
SELECT		
			...
			crmTRN.crmTRN_ValueDate					AS PostDate,
...
SELECT		DAY(crmTRN.crmTRN_ValueDate) as [MonthDay] ,
	...
	AND (MONTH(crmTRN.crmTRN_ValueDate) =  MONTH(GetDate()) -1 )
	AND YEAR(crmTRN.crmTRN_ValueDate) = YEAR(GetDate())
	group by crmCAMP.crmCAMP_CodeName, MLT_TRN_Type.crmMLT_TextInLanguage, crmTRN.crmTRN_ValueDate
    ORDER BY  crmCAMP.crmCAMP_CodeName, MLT_TRN_Type.crmMLT_TextInLanguage, crmTRN.crmTRN_ValueDate
"@
